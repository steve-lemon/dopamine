...
@gin.configurable
def create_retro_environment_exam(game_name='BubbleBobble', sticky_actions=True, level=1):
    '''create retro game'''
    print('! create_retro_environment in retro_lib_exam.py: %s/%d' % (game_name, level))
    env = RetroPreprocessingExam.createRetroGameEnv(game_name, sticky_actions, level)
    return RetroPreprocessingExam(env, game_level=1)


@gin.configurable
class RetroPreprocessingExam(object):
    '''RetroPreprocessing
    ...
    def createRetroGameEnv(game_name='BubbleBobble', sticky_actions=True, level=1):
        ...
        tf.logging.info('Create RetroGame in RetroPreprocessingExam:%s w/ stage:%s' % (full_game_name, state))
        # close the previous one
        if RetroPreprocessingExam._env is not None:
            RetroPreprocessingExam._env.close()
            RetroPreprocessingExam._env = None
        # make env, then cache it.
        env = retro.make(game=full_game_name, state=state)
        RetroPreprocessingExam._env = (env if env else None)
        return env

    def __init__(self, environment, frame_skip=4, terminal_on_life_loss=False, screen_size=84, wall_offset=200, step_penalty=-0.001, game_level=0, reset_fire=0, score_bonus=0):
        ...
        self.episode_true_return = 0

        print('! RetroPreprocessingExam: wall_offset={}, step_penalty={}, game_level={}, reset_fire={}, score_bonus={}'.format(self.wall_offset, self.step_penalty, self.game_level, self.reset_fire, self.score_bonus))
    ...
    def reset(self, game_level = 0):
        ...
        if game_level > 0 and game_level != self.game_level:
            self.environment = RetroPreprocessingExam.createRetroGameEnv(level = game_level)
    ...
    def step(self, a):
        ...
        for time_step in range(self.frame_skip):
            ...
            self.episode_true_return += reward
